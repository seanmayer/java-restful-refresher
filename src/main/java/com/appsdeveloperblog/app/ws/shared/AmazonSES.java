...
public class AmazonSES {
  ...
  public void verifyEmail(UserDto userDto) {
    ...
    String textBodyWithToken = TEXTBODY.replace(
      "$tokenValue",
      userDto.getEmailVerificationToken()
    );
  ...
  public boolean sendPasswordResetRequest(
    String firstName,
    String email,
    String token
  ) {
    ...
    String textBodyWithToken = PASSWORD_RESET_TEXTBODY.replace(
      "$tokenValue",
      token
    );
    ...
    SendEmailRequest request = new SendEmailRequest()
      ...
      .withMessage(
        new Message()
          ...
          .withSubject(
            new Content().withCharset("UTF-8").withData(PASSWORD_RESET_SUBJECT)
          )
      )
      .withSource(FROM);
